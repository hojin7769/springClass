...
import org.sist.web.newlecture.service.MemberShipService;
...
public class CustomerController {
	...
	*/
	...
	public String noticeDel(String seq , HttpServletRequest request
		...
		int resultCount = mybatisNoticeDao.delete(seq);
	...
	public String noticeEdit( Notice notice,
		...
		if (!multipartFile.isEmpty()) {  //수정 첨부파일 O
			...
			uploadPath = request.getSession().getServletContext().getRealPath("/customer/upload");
		...
		NoticeDao mybatisNoticeDao =  this.sqlSession.getMapper(NoticeDao.class);
		int resultCount = mybatisNoticeDao.update(notice);
	...
	//Post
	//2 java.security.Principal 클래스
		@RequestMapping(value = {"noticeReg.htm"},method = RequestMethod.POST )
		public String noticeReg( Notice notice , HttpServletRequest request , Principal principal ) throws Exception {
			//첨부된 파일 유무를 확인해서...
			CommonsMultipartFile multipartFile =  notice.getFile();
			...
			this.memberShipSerivce.insertAndPointUpOfMember(notice, principal.getName());
			//
